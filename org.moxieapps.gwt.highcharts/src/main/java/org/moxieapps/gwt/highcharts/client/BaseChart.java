...
public abstract class BaseChart<T> extends Widget {
    ...
    /**
     * Updates just the height of the chart to the given pixel height, leaving the width untouched,
     * using the default animation options.
     *
    ...
    /**
     * Updates just the height of the chart to the given pixel height, leaving the width untouched,
     * explicitly setting whether or not the resize should be animated.
     *
     * @param height   The new pixel height of the chart.
     * @param animated When true, the resize will be animated with default animation options.  The animation
     *                 can also be configured by call the {@link #setHeight(int, Animation)} method instead.
     * @return A reference to this {@link BaseChart} instance for convenient method chaining.
     */
    public T setHeight(int height, boolean animated) {
        return setHeight(height, animated ? new Animation() : null);
    }

    /**
     * Updates just the height of the chart to the given pixel height, leaving the width untouched, allowing for custom control
     ...
     * @return A reference to this {@link BaseChart} instance for convenient method chaining.
     */
    public T setHeight(int height, Animation animation) {
        ...
        if (isRendered()) {
            ...
            if (animation == null || animation.getOptions() == null) {
                nativeSetHeight(chart, height, animation != null);
            } else {
                nativeSetHeight(chart, height, animation.getOptions().getJavaScriptObject());
            }
        } else {
            // In we're invoked before the chart is rendered, keep the height around in our configuration
    ...
    /**
     * Updates just the width of the chart to the given pixel width, leaving the height untouched,
     * using the default animation options.
     *
     ...
     * @param width    The new pixel width of the chart.
     * @param animated When true, the resize will be animated with default animation options.  The animation
     *                 can also be configured by call the {@link #setWidth(int, Animation)} method instead.
     * @return A reference to this {@link BaseChart} instance for convenient method chaining.
     */
    public T setWidth(int width, boolean animated) {
        return setWidth(width, animated ? new Animation() : null);
    }

    /**
     * Updates just the width of the chart to the given pixel width, leaving the height untouched, allowing for custom control
     * over how the animation will be resized.
     *
     * @param width    The new pixel width of the chart.
     * @param animation The custom animation to use as the resize animation for the chart.
     * @return A reference to this {@link BaseChart} instance for convenient method chaining.
     */
    public T setWidth(int width, Animation animation) {
        // Avoid monkeying with anything if we got bad values
        if (width <= 0) {
            return returnThis();
        }
        ...
        if (isRendered()) {
            // If we're already on the screen, then do the resize natively
            if (animation == null || animation.getOptions() == null) {
                nativeSetWidth(chart, width, animation != null);
            } else {
                nativeSetWidth(chart, width, animation.getOptions().getJavaScriptObject());
            }
        } else {
            // In we're invoked before the chart is rendered, keep the width around in our configuration
    ...
    private static native void nativeSetHeight(JavaScriptObject chart, int height, boolean animated) /*-{
        chart.setSize(undefined, height, animated);
    }-*/;

    private static native void nativeSetHeight(JavaScriptObject chart, int height, JavaScriptObject animation) /*-{
        chart.setSize(undefined, height, animation);
    }-*/;

    private static native void nativeSetWidth(JavaScriptObject chart, int width, boolean animated) /*-{
        chart.setSize(width, undefined, animated);
    }-*/;

    private static native void nativeSetWidth(JavaScriptObject chart, int width, JavaScriptObject animation) /*-{
        chart.setSize(width, undefined, animation);
    }-*/;
